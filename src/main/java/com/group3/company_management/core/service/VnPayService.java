...
public class VnPayService {
    ...
    @Transactional
    public void confirmReviewedPayment(Long invoiceId) {
        Invoice inv = invoiceRepository.findById(invoiceId)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy hóa đơn."));
        if (inv.getStatus() != Invoice.InvoiceStatus.PAYMENT_PENDING) {
            throw new RuntimeException("Chỉ hóa đơn đang chờ xử lý thanh toán mới được xác nhận hoàn tất.");
        }
    ...
    private String query(Map<String, String> p) {
        return p.entrySet().stream().filter(e -> e.getValue() != null && !e.getValue().isBlank()).map(e -> enc(e.getKey()) + "=" + enc(e.getValue())).reduce((a, b) -> a + "&" + b).orElse("");
    }
        ...
        try {
            if (inv.getContract() != null && inv.getContract().getCustomer() != null) {
                notificationService.createCustomerNotification(
                    ...
                    "Hóa đơn số " + inv.getInvoiceCode() + " đã được xác nhận thanh toán thành công số tiền " + String.format("%,.0f", inv.getTotalAmount()) + " VNĐ."
                );
            }
            ...
            if (inv.getContract() != null && inv.getContract().getCustomer() != null) {
                ...
                if (customerEmail != null && !customerEmail.isBlank()) {
                    ...
                    String content = String.format("""
                            ...
                            Chúng tôi xác nhận đã hoàn tất xử lý thanh toán cho hóa đơn %s thuộc hợp đồng %s.
                    ...
                    log.info("Đã gửi email xác nhận thanh toán hóa đơn {} tới {}", inv.getInvoiceCode(), customerEmail);
                }
            }
            ...
            if (inv.getContract() != null && inv.getContract().getSale() != null) {
                notificationService.createNotification(
                        ...
                        "Hóa đơn " + inv.getInvoiceCode() + " thuộc hợp đồng " + inv.getContract().getContractCode() + " đã được xác nhận thanh toán số tiền " + String.format("%,.0f", inv.getTotalAmount()) + " VNĐ."
                );
            }
        } catch (Exception e) {
            log.error("Lỗi gửi email hoặc thông báo thanh toán hóa đơn: {}", e.getMessage());
        }
    }
}
